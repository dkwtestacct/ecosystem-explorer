...
1. **Phase 0 — investigate first.** Before any non-trivial change, read the code and confirm the brief's premises against live state. Sentinel checks ("X should already exist", "Y should not yet appear") catch wrong premises before they ship. When a sentinel fires, reconcile to the brief's *intent* against live state and report what you found — halt for direction only if the right action is genuinely unclear (per rule 2, report ≠ hold).
2. **The machine gate is the only gate.** `verify_baselines.py` 40/40 green + the brief's acceptance assertions are the whole gate — there is **no pre-commit hold and no separate pre-push hold**, not even for honesty-surface / validation / cross-doc / first-of-a-kind changes. **On green → commit + push immediately, then continue** to the next concern. **On red → stop and report the failing baseline(s)** — an intentional output shift reads as red until `--update`, and that red state is where drift review happens (see rule 5 and `DESIGN_NOTES.md` red-path discipline). The human eyeball is **async**: it becomes a follow-up brief only if something actually renders wrong. Still investigate-first (rule 1) and still make + *report* judgment calls when premises drift or you fold in a correction — but report ≠ hold; shipping a fix beats sitting on it.
...
7. **Push on green — it's part of the gate, not a separate user action.** When the machine gate passes (rule 2), `git push` immediately after the commit; do not wait for a human "go." The only thing that stops a push is a red gate. (This reverses the earlier "push is a user-only action" policy.)
...
- **Stop-and-report sentinels are signal, not noise.** Briefs include sentinel checks ("X should not already exist", "Y should match expected content") for a reason. When a sentinel fires, the brief's precondition isn't met — surface what was found and reconcile to the brief's *intent* (don't skip the check or reframe content as "close enough"). Halt for direction only when the correct action is genuinely ambiguous; otherwise make the call, report it, and let the machine gate arbitrate (rule 2).
...
- **WHATS_NEW + Underway discipline.** `WHATS_NEW_SECTIONS` (list of `(title, [entries])`) groups **blockbuster user-facing capabilities only — brief, one line each, no minor or internal changes**, into capability sections (e.g. "Interactive scenario placement", "Validation and handoff"). The bar: the change has *already shipped*, would be noticed by a returning user within ~7 days, and is the kind of thing the user wants to know they can now do (a new feature, a new city, a major UI surface). Excluded: doc-only changes, internal refactors, methodology validations whose only user-visible effect is a confidence-badge upgrade, framing/labeling tweaks, and data/model-update implementation history. When in doubt, cut. `UNDERWAY_ENTRIES` is forward-looking work the user will recognize when they see it (a new model, a new city, a UI feature). Both default to empty; populate only when a change clears the bar.
