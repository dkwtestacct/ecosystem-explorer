...
2. **Stop-and-report before commit.** For anything substantive — rewrites, multi-step builds, anything that crosses doc boundaries — surface the draft + sentinel results + open questions, then wait for explicit approval. Never assume "small enough" — the user gates by surface, not by line count.
...
- **WHATS_NEW + Underway discipline.** `WHATS_NEW_ENTRIES` are user-visible changes that have *already shipped* and would be noticed by a returning user within ~7 days (no doc-only changes, no internal refactors). `UNDERWAY_ENTRIES` is forward-looking work the user will recognize when they see it (a new model, a new city, a UI feature). Both default to empty; populate only when a change clears the bar.
