...
**Multi-city support.** Two Minneapolis views are currently available — a downtown 122.8 km² view (EPSG:26915, InVEST sample buildings with per-type codes) and a full-city 204.3 km² view (EPSG:5070, OSM buildings without per-type codes). Each city has its own `CITIES` config entry declaring all input paths and CRS; switching between cities is a sidebar interaction with separate cached lookup tables. The schema is forward-compatible — adding a new city is a config-only operation provided its input rasters and shapefiles exist on disk.
...
**Census Tracts** — TIGER 2020 census tracts. For downtown Minneapolis, 27 tracts intersecting the InVEST sample area; for Minneapolis Full, all 329 Hennepin County tracts (`process_tracts_expanded.py`). Used for the per-tract neighborhood-improvement reporting overlay on the Map View tab.

**Reference Evapotranspiration** — Annual ET raster from the InVEST UCM sample data (1 km resolution, native ~10 × 10 km extent), bilinear-resampled to whichever city's NLCD grid is active. The same raster covers both Minneapolis cities; for Minneapolis Full it bilinear-extrapolates beyond its native extent at the bbox corners, which is acceptable for an order-of-magnitude metric. The raster's nodata sentinel (65535) is masked before resize so the InVEST CC formula's ETI term works correctly.
