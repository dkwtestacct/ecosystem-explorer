...
| **Model quality mode** | Radio with three options — `Fast prototype` (default), `Balanced`, `High resolution` — written to `st.session_state['model_quality']`. Selects the surrogate's training set and tree count. **Fast prototype** uses `compute_scenario_grid(step_pct=10, step_alloc=25)` → ~90 scenarios, 100 trees. **Balanced** prefers `data/scenarios_dense.csv` (built offline by `precompute_scenarios.py`, ~726 scenarios at `step_pct=5, step_alloc=10`) and falls back to recomputing on the fly with a warning if the CSV is missing; 200 trees. **High resolution** reuses the full 2,541-entry lookup table as training data — no extra simulation needed since those entries are already computed for instant slider response — and uses 300 trees. The selection is passed to `train_surrogate` as `mode_key` and `n_estimators`, both of which key its `@st.cache_resource`, so flipping the radio automatically retrains without a manual cache clear. A sidebar caption shows the loaded count and tree number ("Active: N training scenarios, K trees"). |

**Three knobs, three roles:** the surrogate has *training scenarios* (how many full-resolution simulations the Random Forest learns from), *trees* (RF ensemble size — more trees = smoother predictions, tighter uncertainty bands), and *optimizer candidates* (~10,000 random scenarios sampled at optimization time and scored against the trained surrogate). Model quality mode adjusts the first two; the optimizer candidate count is independent and unchanged across modes.
