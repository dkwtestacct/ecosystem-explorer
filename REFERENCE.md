...
| **Caveats** | **(1) NDVI is synthetic** — replace with satellite NDVI for site-specific work. **(2) Box-filter not Gaussian** — InVEST canonically uses a Gaussian kernel with the `search_radius_m` value as σ; we use `scipy.ndimage.uniform_filter` (cheaper, similar mean preservation). **(3) Lifetime vs annual prevalence** — using ever-diagnosed CDC numbers as the at-risk pool may overstate the actual annual incidence the scenario would reduce. **(4) Static population** — the scenario doesn't model in-migration that would follow improved amenities. **(5) Direct exposure pathway only** — omits secondary mechanisms like air-quality and social-cohesion effects. **(6) Cost-of-illness numbers are nominal** — published meta-analyses range ~$5K–$15K depending on healthcare-system context. Replace with locally-calibrated values for production work. |
