...
| **Discover scenarios to validate** *(no filter active)* | Sliders for minimum flood / cooling / food / carbon plus a runoff cap, then the **Optimize** button that scores the citywide surrogate over ~10,000 random `(pct, GI%, FF%)` candidates and returns up to 5 suggestions ranked by a balanced score. Predicted values, not engine outputs — Apply a suggestion to run the full engine. The orange P10/P90 bands on the Tradeoff chart and in the optimizer table reflect the surrogate's per-tree disagreement (see §8). |
| **Discover scenarios to validate** *(region or ownership filter active)* | Same sidebar section, but the **Optimize selected area** mode replaces the min-target sliders with five **weight sliders** (Cooling / Flood reduction / Carbon / Food / Cost). Clicking the button runs a two-stage search: a fast surrogate (Phase-0.5-validated Fast config — 90 recipes, 100 trees) shortlists ~40 Pareto-efficient candidates, then the full engine evaluates each shortlisted recipe on the active region∩ownership mask. The returned top 5 carry **engine-true region-local values** — not predictions, no P10/P90 bands. Header above the table reads *"Top scenarios found — selected area"*, framing them as candidates rather than a global optimum because the shortlist may not be exhaustive. Apply on one of the 5 sets the sliders, reruns the engine on the active mask, and flips provenance to **"Engine-verified — region-optimized"** (distinct from the citywide "Surrogate-suggested"). See §8. |
