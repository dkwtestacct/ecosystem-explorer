...
| Temperature Change | Mean Gaussian-smoothed CC as HMI proxy, `ΔCC × UHI_MAX_C × 1.8` | Urban Cooling (HMI → T_air → anomaly) | Approximate | High |
| Cooling Energy Savings | `consumption_rate × ΔCC × UHI_MAX_C × pixel_area × $/kWh` | Urban Cooling (energy module: `consumption × ΔT_air × $/kWh`) | Approximate | Medium |
...
InVEST UCM computes per-pixel Cooling Capacity (`CC = 0.6·shade + 0.2·albedo + 0.2·ETI`), then derives a Heat Mitigation Index (HMI) that accounts for large green spaces (≥2 ha) within `d_cool` meters via exponential distance-weighted averaging of park CC values. HMI feeds into air temperature estimation (`T_air = T_ref + (1 − HMI) × UHI_max`), which is then Gaussian-smoothed for air mixing, and finally into per-building energy savings (`consumption × ΔT_air × $/kWh`). The app implements the CC formula faithfully and applies Gaussian smoothing at 450 m, but **skips the HMI step entirely** — mean smoothed CC is used as the HMI proxy. Two consequences: (a) the park-cooling-plume effect (large green spaces cooling beyond their footprint via exponential decay) is replaced by a simpler Gaussian blur, and (b) the per-building T_air sampling over a 600 m blending radius is not performed. The energy savings formula uses `ΔCC × UHI_MAX_C` as the temperature delta rather than the canonical `T_air_MAX − T_air_i` per building. The InVEST guide flags that the CC weights (0.6/0.2/0.2) have "high uncertainties" and that `d_cool` and the air-blending radius should be calibrated against observed data.
...
**Heat Mitigation (HM) Index** — A 0–1 index measuring how well a land cover type reduces urban heat through shade and evapotranspiration (cooling via water evaporation and plant transpiration). A score of 0 means no mitigation (bare pavement); a score near 1 means strong cooling effect (dense vegetation). In InVEST's canonical Urban Cooling Model, HMI accounts for nearby park cooling via distance-weighted CC averaging; the app uses Gaussian-smoothed mean Cooling Capacity (CC) as an approximation (see "Official InVEST alignment — UCM").
...
| **Note on labeling** | The app reports mean Cooling Capacity (CC) across the model area, which approximates but is not identical to the InVEST Heat Mitigation Index (HMI). Both are bounded 0–1 and respond to the same biophysical inputs, but canonical HMI = `max(CC_local, CC_park)` where `CC_park` is a distance-weighted average of CC values from green areas ≥2 ha within `d_cool` meters (exponential decay). The app's Gaussian-smoothed mean CC substitutes a simpler spatial blending for this park-proximity step. Two additional InVEST UCM steps remain unimplemented: (a) air temperature interpolation (`T_air = T_ref + (1 − HMI) × UHI_max` with Gaussian air-mixing), and (b) per-building T_air sampling over a 600 m blending radius. See [`UCM_AUDIT.md`](data/invest/cooling/UCM_AUDIT.md) for the full divergence log and the "Official InVEST alignment" section above for the parity rating. |
...
| **Caveats** | The 3.69 °F/CC factor comes from MN's `uhi_max=2.05 °C`; treat the °F output as ±2 °F at best, since wind, humidity, urban geometry, and anthropogenic heat are not modelled. Three canonical InVEST UCM steps remain unimplemented: (a) the HMI park-proximity step (distance-weighted CC averaging from green areas ≥2 ha — we substitute Gaussian smoothing), (b) air temperature interpolation (`T_air = T_ref + (1 − HMI) × UHI_max` with air-mixing convolution), and (c) per-building T_air sampling over a 600 m blending radius. The Gaussian convolution we *do* apply gets part of the way there: buildings adjacent to (not just on top of) converted pixels receive cooling credit. See "Official InVEST alignment — UCM" for the full gap description. |
...
| **Caveats** | (1) **Per-pixel, not per-building polygon** — we don't run InVEST's 600 m `t_air_average_radius` step that would average T_air across each building footprint before applying the consumption rate; for buildings spanning a CC discontinuity this could shift the dollar number. (2) **Building-extent limited** — coverage is the InVEST sample-data buildings shapefile; areas of the city outside that shapefile contribute zero. (3) **Annual flat rate** — consumption rates are annualised in the InVEST table; we do not apply a cooling-degree-day weighting. (4) **ΔCC-based, not T_air-based** — InVEST UCM's canonical energy formula uses per-building `T_air_MAX − T_air_i`; the app uses `ΔCC × UHI_MAX_C` as the temperature delta, which is a reasonable approximation but not identical. MN uses `UHI_MAX_C = 2.05 °C` (from InVEST args JSON); SA uses `3.5 °C` (estimated for Köppen BSh). Treat the dollar figure as order-of-magnitude. |
