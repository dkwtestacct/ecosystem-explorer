...
| **`■ InVEST-validated`** | teal | An everyday-view card whose InVEST model has measured per-pixel parity against canonical `natcap.invest` 3.19.0 via a committed comparison harness (read from `model_validation.VALIDATED_MODELS`) AND is on the validated compute path: Temperature/cooling (UCM), Preventable MH (UMH), Runoff Retention (UFR), and SA four-pool Carbon. (Nature Access / UNA is `InVEST-aligned` pending its per-pixel parity reproducer — see §6.) |
...
- **Validation status** — `InVEST-aligned`. Canonical 2SFCA method (`calculate_nature_access` is a faithful numpy port of `natcap.invest.urban_nature_access`); **per-pixel parity reproducer in progress.** The prior "validated per-pixel parity" claim is withdrawn — it had no committed 3.19.0 reproducer (the only UNA harness was a 3.16.2 reachability-proxy comparison measuring a different statistic, and the r = 1.0 figure came from an InVEST-vs-itself byte-identical-LULC run, not a test of the numpy port). Re-promotes to `InVEST-validated` when a matched-but-independent `urban_nature_supply_percapita` reproducer + `comparisons/` artifact land.
...
- **Validation status** — `InVEST-aligned`. Inherits Nature Access's canonical-InVEST 2SFCA method basis (per-pixel parity reproducer pending, as for the parent metric); the child reweight touches only the final share, so no separate per-pixel parity claim. Not a surrogate target — computed deterministically (like UMH), so it carries no P10/P90 band.
...
- **Validation status** — `InVEST-aligned`. Reuses Nature Access's canonical-InVEST 2SFCA method basis (per-pixel parity reproducer pending, as for the parent metric). Different *quantity* (count of school points, not population-weighted), same *method*; sampling at point locations is the only addition, not a new model. Not a surrogate target — computed deterministically.
