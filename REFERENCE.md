...
InVEST UMH (added in v3.19.0) computes preventable mental health cases via `PC = (1 − RR) × BIR × POP`, where `RR = exp(ln(RR₀.₁) × 10 × ΔNE)` and NE is a neighborhood-mean NDVI within a user-specified search radius. The app follows this formula closely. Two minor divergences: (a) InVEST takes BIR per administrative unit from a vector input (allowing spatial variation in baseline prevalence); the app uses uniform national CDC rates. (b) InVEST computes NE as a uniform **buffer-mean** within the search radius; the app uses **Gaussian** smoothing (`σ = search_radius / pixel_size`) — a different kernel (Gaussian weights decay smoothly and reach ~2-3× further than the flat disk). **Validation (`compare_umh_invest.py`, 2026-05-29):** the app's per-pixel preventable-cases output was compared against `natcap.invest.urban_mental_health.execute()` (v3.19.0) on identical NDVI/population inputs (uniform CDC rate, per-outcome effect size). **City-wide totals agree to ~1.3–1.5 %** (both kernels roughly conserve total exposure), but **per-pixel agreement is Pearson r ≈ 0.95 (MN) – 0.98 (SA)**, the residual driven entirely by divergence (b). Switching the app to a buffer-mean kernel for exact per-pixel parity is a deferred follow-up (it would shift `preventable_mh_cases` everywhere → baseline + dense-CSV regen + schema bump). The validation used the **synthetic NDVI proxy**, so it confirms the *algorithm*, not the NDVI source. A more significant limitation for site-specific work: the app feeds **synthetic per-NLCD-class NDVI** rather than satellite-derived NDVI, so the NE raster is a proxy. The InVEST guide flags that the model "only measures availability; does not capture accessibility or intensity of contact" and excludes blue-space effects.
