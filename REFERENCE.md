...
| Nature Quality Score | Pop-weighted mean of 0–1 access score — reachability, no demand term (Phase 1: proxy ~70 % access vs InVEST ~10 % adequately supplied) | Urban Nature Access (SUP_DEMadm_cap) | Proxy | Medium |
...
**Phase 1 comparison (2026-05-21).** `compare_una_invest.py` ran the MN baseline through both methods — the app's proxy and canonical `natcap.invest.urban_nature_access.execute()` (3.16.2, NatCap's published MN config: uniform 1,000 m radius, exponential decay, 250 m²/capita demand). Spatially the two metrics are only moderately correlated: **Pearson r ≈ 0.44, Spearman r ≈ 0.46, min-max-normalized MAE ≈ 0.51** across 70,868 valid pixels. The headline divergence is starker: the proxy reports **~70 % of residents with nature access**, while canonical 2SFCA finds only **~10 % adequately supplied** against the 250 m²/capita demand (mean per-capita balance is negative — net undersupply). The two tell opposite stories because the proxy has **no demand term** — it measures whether nature is *reachable*, never whether the *supply is adequate for the population competing for it*, which is the whole point of 2SFCA. (A 100 % binary "is any nature reachable" agreement also holds, but it is near-trivial in dense downtown MN — essentially every pixel has some nature within 1 km, so the metrics diverge on magnitude and per-capita adequacy, not on binary reachability.) This is a **fundamental metric-definition mismatch, not a numerical offset**: closing it would mean adopting 2SFCA as the Nature Access metric, not refining the existing proxy. Phase 1 measured the gap; the gap remains open. Full numbers in `comparisons/una_baseline_mn.csv`.
