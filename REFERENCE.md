...
- **Validation status** — `InVEST-validated`. **Measured per-pixel parity:** the numpy `calculate_nature_access` matches `natcap.invest.urban_nature_access.execute()` 3.19.0 on `urban_nature_supply_percapita` at Pearson r = 1.000000, per-pixel MAE 0.054 m²/person (~5.5e-7 of the ~99,000 m²/person field), |Δtotal| ≈ 0 over 70,868 MN px (`validation/compare_una_supply_invest.py` → `comparisons/una_supply_parity_mn.csv`). **Matched-but-independent** — InVEST computes its own supply from the same LULC / population / attribute-table / params, *not* InVEST-vs-itself (the flaw in the retired byte-identical-LULC artifact) — with a non-vacuous +2%-population guard. Supersedes the earlier withdrawn ungrounded per-pixel claim and the 3.16.2 reachability proxy.
