...
| **Discover scenarios** *(no filter active)* | Card titled *Discover scenarios* with mode label **Citywide machine-learning search**. Sliders for minimum flood / cooling / food / carbon plus a runoff cap, then the **Optimize** button that scores the citywide surrogate over ~10,000 random `(pct, GI%, FF%)` candidates and returns up to 5 suggestions ranked by a balanced score (header: *Suggested scenarios*). Predicted values, not engine outputs — Apply a suggestion to run the full engine. The orange **estimate ranges** on the Tradeoff chart and in the optimizer table are the calibrated 10th–90th residual interval (see §8). |
...
| **`△ Prototype`** | gray | A `prototype` metric (no canonical InVEST analog) in any context, plus MN carbon (proxy, not the four-pool stock model). The leading glyph (◆ ■ ○ △) on every tier is a shape-distinct, hue-independent marker — the four tier colors share near-identical grayscale luminance, so the glyph keeps them distinguishable for color-blind viewers. |

**Context-switch rule.** `NatCap published value` only fires when the dashboard is in the fixed-scenario reference view *and* a NatCap published number exists for that metric. In every other scenario context (Baseline / Explorer-generated / machine-learning suggestion), the card shows its everyday tier — `InVEST-validated` for the parity-measured models, otherwise `InVEST-aligned` — because the displayed value is the prototype's own computation, not NatCap's published number.
...
- **Units** — °F (vs baseline).
- **Validation status** — `InVEST-validated`. **Measured per-pixel parity:** the HMI raster matches `natcap.invest.urban_cooling_model.execute()` at MAE = 0.0000, Pearson r = 1.0000 on the MN baseline (`compare_ucm_invest.py`). One remaining UCM divergence affects only Cooling Energy Savings (see §8 for the gap), not this Temperature Change card, which is a direct function of the canonical HMI raster.
...
- **Units** — acre-feet.
- **Validation status** — `InVEST-aligned`. Canonical SCS-CN. No NatCap citywide reference for direct comparison.
...
- **Validation status** — `InVEST-validated`, **with measured per-pixel parity.** This is literally InVEST UFR's canonical `rnf_rt_idx`, and it is **validated to per-pixel parity** against `natcap.invest` UFRM 3.19.0's `runoff_retention_index`: per-pixel MAE ≈ 0 (5e-8) / r = 1.0 over 3.36 M pixels on value-identical CN (`validation/compare_ufr_invest.py` → `comparisons/ufr_sa_retention_parity.csv`, Relay 71). UFRM's λ=0.2 / `S_max=25400/CN−254` mm is algebraically identical to the evaluator's `Ia=0.2·S` / `S=1000/CN−10` in (the mm-vs-inch scale cancels in the dimensionless `Q/P`). The badge stays aligned-method (blue) — there is **no published NatCap SA flood value** to match against (Vibrant Land used UFRM without damage valuation), so it isn't a NatCap-match (green) — but the tooltip now cites the measured parity. Note this validation covers the **per-pixel** retention index only; the headline **Flood Index** (`100 − mean_CN`) and **Runoff Volume** are lumped mean-CN proxies, not per-pixel UFRM outputs, and stay aligned-method without a parity claim.
...
- **Validation status** — **SA:** `InVEST-validated`, **with measured per-pixel parity.** SA Carbon uses InVEST's canonical four-pool stock framework via NatCap's compound table — aligned with the methodology in NatCap's 2023 Vibrant Land report (Guerry et al.) for the same SA project — and is **validated to per-pixel parity** against `natcap.invest.carbon.execute()` 3.19.0 in matched units: per-pixel MAE ≈ 0 (3.3e-7 Mg C) / r = 1.0 over 3.36 M pixels (`validation/compare_carbon_sa_fourpool_invest.py` → `comparisons/carbon_sa_fourpool_parity.csv`). InVEST's `c_storage` raster is per-hectare density and the evaluator reports per-pixel total (× pixel-area-ha 0.09); normalizing reconciles them exactly. **MN:** `Prototype`. No NatCap MN four-pool bundle is available; the per-cover-class annual rate is provisional.
...
- **Validation status** — `InVEST-validated`. **Measured per-pixel parity:** the numpy `calculate_nature_access` matches `natcap.invest.urban_nature_access.execute()` 3.19.0 on `urban_nature_supply_percapita` at Pearson r = 1.000000, |Δtotal| ≈ 0 (AOI-sum diff 0.00001%) over 70,868 MN px (`validation/compare_una_supply_invest.py` → `comparisons/una_supply_parity_mn.csv`). The per-pixel MAE of 0.054 m²/person is **float-precision noise** on the ~99,000 m²/person field (float32 raster storage + independent convolution rounding — app `fftconvolve` vs InVEST `pygeoprocessing.convolve_2d`): the app-vs-InVEST fit is `app = 1.0000·invest + 0.02` (slope = 1, intercept ≈ 0 — no scale or offset), and the residual is ~98% grid-interior with only mild AOI-boundary elevation. **Matched-but-independent** — InVEST computes its own supply from the same LULC / population / attribute-table / params, *not* InVEST-vs-itself (the flaw in the retired byte-identical-LULC artifact) — with a non-vacuous +2%-population guard. Supersedes the earlier withdrawn ungrounded per-pixel claim and the 3.16.2 reachability proxy.
...
- **Validation status** — `InVEST-aligned`. Inherits Nature Access's canonical-InVEST 2SFCA validation (per-pixel parity vs InVEST 3.19.0, `compare_una_supply_invest.py`); the child reweight touches only the final share, so no separate per-pixel parity claim. Not a surrogate target — computed deterministically (like UMH), so it carries no P10/P90 band.
...
- **Validation status** — `InVEST-aligned`. Reuses Nature Access's canonical-InVEST 2SFCA validation (per-pixel parity vs InVEST 3.19.0, `compare_una_supply_invest.py`). Different *quantity* (count of school points, not population-weighted), same *method*; sampling at point locations is the only addition, not a new model. Not a surrogate target — computed deterministically.
...
- **How it is computed** — Per pixel: ① `NE = _convolve_edge_corrected(NDVI_proxy, disk(r))` — the canonical **edge-corrected buffer-mean** of NDVI over a flat binary circular **disk** of radius `r = UMH_SEARCH_RADIUS_M / PIXEL_SIZE_M = 300 m / 30 m = 10 px` (the canonical InVEST UMH kernel; an edge-corrected mean over the circular disk, **not** a square box filter). ② `ΔNE = NE_scenario − NE_baseline`. ③ `RR = exp(ln(RR_0.1) × 10 × ΔNE)`. ④ `PF = 1 − RR` (preventable fraction). ⑤ `PC = PF × baseline_prevalence × population_count`. ⑥ Sum over valid pixels for depression and anxiety; multiply by per-case cost-of-illness for the dollar metric. Constants: `RR_0_1_NDVI = 0.96` depression / `0.97` anxiety (Liu et al. 2023), `BIR = 0.21` / `0.19` (CDC 2023 ever-diagnosed), cost-of-illness `$8,467` / `$5,765` (US nominal). Population: Census 2020 block-level totals rasterized to the active city's grid.
- **Units** — Cases/yr · USD/yr.
- **Validation status** — `InVEST-validated`. **Measured per-pixel parity:** matches `natcap.invest.urban_mental_health.execute()` (v3.19.0) on identical NDVI / population inputs — **MN** MAE(active) ≤ 1.1e-9 cases/px, r = 1.000000; **SA** MAE(active) ≤ 2.3e-6 cases/px, r ≥ 0.99875, |Δtotal| ≤ 0.15% — the SA residual is canonical's radius padding + edge-crop alignment + pygeoprocessing FFT noise on the 1713 × 1984 grid, not a metric divergence (`validation/compare_umh_invest.py`). The validation used the synthetic NDVI proxy — it confirms the *algorithm*, not the NDVI source.
...
- **Validation status** — `InVEST-aligned`. Canonical SCS-CN underneath; MN dollar approach is a proportional scaling rather than InVEST UFR's per-watershed `serv_blt` indicator (which the InVEST docs themselves call "only an indicator of service, not an actual measure of damage or savings").
...
- **Validation status** — **MN:** `Prototype` (inherits from MN Carbon). **SA:** `InVEST-aligned` (a dollar valuation derived from the validated Carbon stock, not itself a per-pixel parity output). The methodology matches NatCap's Vibrant Land report convention; the SC-CO2 vintage differs (EPA 2023 vs Vibrant Land's IWG 2021 value) — *methodology alignment, current parameter vintage*.
...
- **Flood is ~scenario-invariant.** NatCap's published SA finding (Vibrant Land + supporting presentations): the citywide flood metric is nearly insensitive to scenario choice — developed land is a small share of the county-area extent, so the total-metric movement under any realistic green-conversion scenario is small. The dashboard echoes this — the directionality of greening is real, but the headline number won't move much. Which conversion most reduces runoff is per-city-table dependent: food forest gives a real forest-infiltration benefit, while green infrastructure's flood effect depends on the per-city wetland curve number and is not claimed as a GI benefit (see `docs/internal/DESIGN_NOTES.md` §6.9). Read the Flood Index card with this scope effect in mind.
...
**Estimate range (calibrated 10th–90th) — scope.** The orange-diamond error bars on the Tradeoff Space chart and the range columns in the optimizer-result table are a **calibrated 10th–90th empirical interval**: `[estimate + p10, estimate + p90]`, where p10/p90 are the percentiles of the surrogate's prediction residual (`engine_true − surrogate_pred`) measured by 10-fold held-out cross-validation against the engine grid (`scripts/calibrate_surrogate_band.py` → `data/<city>/surrogate_calibration_<mode>.json`). Because the residual brackets the engine truth, a systematic under-prediction (carbon / food) shows as an upward skew rather than a shifted point estimate. This replaces the earlier inter-tree disagreement band, which was not validated against engine values. **Scope is Mode 1 citywide only:** the live metric cards, the validated per-pixel engine, the displayed-validated taxonomy, AND the Mode 2 region-optimizer results do NOT carry a range — those values are engine-true (the region-no-band lock is asserted in `verify_baselines.py`). **Nature access is suppressed** even in Mode 1 — its true uncertainty is placement-driven, which the mix-only lattice CV understates, so it keeps a placement-sensitive caveat instead of a calibrated range. Modes without a precomputed grid to calibrate against (e.g. MN Fast) show estimates with no range rather than an uncalibrated one.
...
- **Indices vs impacts.** The dashboard surfaces both indices (Flood Index, HMI, NDVI) and impact estimates (acre-feet runoff, °F cooling, dollar damage, preventable cases). Indices are tight; impact estimates carry calibration uncertainty (±2 °F for temperature, order-of-magnitude for dollar metrics) and should be read as planning signals, not engineering numbers.
- **Scenario response is computed from the land-cover map, not live remote sensing.** The tool answers "what changes if this land is reconverted," so the inputs that drive each metric must be functions of the scenario's land-cover classes — otherwise a hypothetical conversion couldn't move the output. UMH greenness, the UCM cooling inputs (shade / albedo / Kc), and the UNA greenspace definition are all land-cover-derived for this reason, so they respond to scenarios. **UMH is the sharpest case** because "NDVI" conventionally connotes satellite data: the UMH NDVI is a per-class synthetic proxy, so its validated badge means "method matches InVEST UMH where tested," not "output matches observed reality." Read absolute UMH levels as indicative and the scenario response (Δ vs baseline) as the signal.
...
- **Estimate range** — calibrated 10th–90th empirical range for citywide machine-learning estimates, derived from cross-validation residuals against evaluator-computed values. Retired: "model disagreement band" / "model disagreement bands", "uncertainty bands". <!-- vocab-allow: glossary names the retired terms -->
