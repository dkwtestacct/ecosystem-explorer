...
- **Validation status** — `InVEST-validated`. **Measured per-pixel parity:** the numpy implementation matches `natcap.invest.urban_nature_access.execute()` at MAE 0.0234 m²/person, Pearson r = 1.000000 on the MN baseline (`validation/compare_una_invest.py`). The aggregate headline `pct_pop_supply_ge_demand` is identical at 46.86 % on the same baseline.
...
- **Validation status** — `InVEST-aligned`. Inherits Nature Access's canonical-InVEST 2SFCA validation (MAE 0.0234 m²/person, MN baseline); the child reweight touches only the final share, so no separate per-pixel parity claim. Not a surrogate target — computed deterministically (like UMH), so it carries no P10/P90 band.
...
- **Validation status** — `InVEST-aligned`. Reuses Nature Access's canonical-InVEST 2SFCA validation (MAE 0.0234 m²/person, MN baseline). Different *quantity* (count of school points, not population-weighted), same *method*; sampling at point locations is the only addition, not a new model. Not a surrogate target — computed deterministically.
