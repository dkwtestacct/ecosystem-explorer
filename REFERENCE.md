...
- **Approximate** — Math is in the spirit of InVEST but takes documented shortcuts (named in the gap notes below).
...
InVEST UNA implements a Two-Step Floating Catchment Area (2SFCA) method: Step 1 computes a nature-to-population ratio for each nature pixel (nature area ÷ decay-weighted population within search radius), Step 2 sums these ratios for each population pixel to produce per-capita supply (m²/person). The model then compares supply against a per-capita demand standard to produce balance metrics (surplus/deficit) per administrative unit. The app uses a simpler approach: per pixel, it computes `max(urban_nature × in_range)` across all nature classes (taking the highest single-class contribution to prevent double-counting), thresholds at 0.3 for Nature Access, and population-weights for Nature Quality Score. This skips the 2SFCA demand/supply/balance framework entirely — the app's output is a proximity-based access indicator, not a supply-demand balance. The InVEST guide notes that "there is no set global standard for urban nature demand" and that Euclidean distance ignores real-world barriers.

**Phase 1 comparison (2026-05-21, denominator-reconciled).** `compare_una_invest.py` ran the MN baseline through both methods — the app's proxy and canonical `natcap.invest.urban_nature_access.execute()` (3.16.2, NatCap's published MN config: uniform 1,000 m radius, exponential decay, 250 m²/capita demand). Spatially the two metrics are only moderately correlated: **Pearson r ≈ 0.44, Spearman r ≈ 0.46, min-max-normalized MAE ≈ 0.51** across the 70,868-pixel modelable extent.

A **denominator caveat** governs the headline shares. The prototype's Nature Access metric scores all 154,242 MN residents — its full-extent figure is **69.7 %** with access. InVEST 2SFCA can only score the *modelable extent* (pixels with valid land cover **and** valid InVEST output), which holds **66,945 residents — 43.4 % of the population**; the other 56.6 % sit on cooling-LULC nodata pixels InVEST cannot model. So the original "≈70 % vs ≈10 %" framing compared two different population bases. Restricted to the **common modelable-extent base**, the proxy reports **100 % of residents with nature access** (every modelable-extent pixel lies within 1 km of some nature class) versus InVEST's **9.5 % with adequate per-capita supply**. Reconciling the denominators **widens** the gap rather than narrowing it: the 69.7 % full-extent proxy figure was *deflated* by ~46,800 off-LULC residents the proxy scored 0 (they sit >1 km from any nature class), so dropping the population InVEST cannot model lifts the proxy side toward 100 %. The mixed-denominator headline understated the proxy, not the gap.

The two metrics tell opposite stories because the proxy has **no demand term** — it measures whether nature is *reachable*, never whether the *supply is adequate for the population competing for it*, which is the whole point of 2SFCA. (A 100 % binary "is any nature reachable" agreement also holds, but it is near-trivial in dense downtown MN — essentially every pixel has some nature within 1 km, so the metrics diverge on magnitude and per-capita adequacy, not on binary reachability.) This is a **fundamental metric-definition mismatch, not a numerical offset**: closing it would mean adopting 2SFCA as the Nature Access metric, not refining the existing proxy. Phase 1 measured the gap; the gap remains open. Full numbers — full-extent and restricted — in `comparisons/una_baseline_mn.csv`; spatial case studies in `UNA_DIVERGENCE_CASE_STUDIES.md`.
...
| **Formula** | Per-pixel cooling capacity from the InVEST Urban Cooling Model: **`CC = 0.6 × shade + 0.2 × albedo + 0.2 × ETI`**, where **`ETI = Kc × (ET_annual / ET_max)`** normalizes actual evapotranspiration to a 0–1 fraction of the model area's maximum. The app then computes the canonical InVEST **Heat Mitigation Index — `HMI = max(CC_local, CC_park)`** — where `CC_park` is the exponentially distance-weighted average of CC from green areas ≥2 ha within `d_cool = 450 m`, applied where a pixel has enough green in reach. Helpers: `_compute_cc_park_raster`, `_compute_green_area_sum`, `_compute_hmi_raster` (convolution via `scipy.signal.fftconvolve` with InVEST-canonical edge correction). The card's reported value is `mean(HMI)` over valid pixels, labeled "Cooling Capacity" / "CC". Temperature change against baseline = `(HMI_scenario − HMI_baseline) × HM_TO_FAHRENHEIT`, where `HM_TO_FAHRENHEIT = UHI_MAX_C × 1.8`. `UHI_MAX_C = 2.05 °C` for MN (from InVEST args JSON); `3.5 °C` for SA (estimate for Köppen BSh climate, no NatCap-published value) — yielding 3.69 °F/CC for MN, 6.30 °F/CC for SA. |
| **Note on labeling** | The app reports mean Heat Mitigation Index (HMI) across the model area — the canonical InVEST UCM output. `HMI = max(CC_local, CC_park)` where `CC_park` is a distance-weighted average of CC values from green areas ≥2 ha within `d_cool` meters (exponential decay). The UI label "Cooling Capacity" / "CC" is retained for continuity; the underlying value is canonical HMI, **validated at MAE = 0.0000, r = 1.0000 against `natcap.invest.urban_cooling_model.execute()`** (see "Official InVEST alignment — UCM"). One InVEST UCM step beyond the HMI raster remains unimplemented: per-building T_air sampling over a 600 m blending radius — this affects *Cooling Energy Savings* dollar magnitudes but not *Temperature Change*. See [`UCM_AUDIT.md`](data/invest/cooling/UCM_AUDIT.md) for the implementation-status writeup and the "Official InVEST alignment" section above for the parity rating. |
...
| Urban Cooling | Cooling Capacity (higher = more cooling) | `BASELINE_HM` — auto-recomputed at module load from the canonical HMI raster (MN baseline mean HMI ≈ 0.1944); the hardcoded value in `CITIES` is a documentation placeholder | `mean_hm` from scenario (canonical HMI) |
