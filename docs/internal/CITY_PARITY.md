...
| Child population raster | Census 2020 PL 94-171 under-18 (`P1_001N − P3_001N`), rasterized by the same uniform-block-spread as the total-pop raster (same blocks, same resolution, same CRS). `config.py:child_pop_file = data/population/minneapolis_child_pop_2020.tif`. Extent share 20.2 % (anchored in verify_baselines). | not in NatCap MN sample bundle | ✅ Configured. Powers the Children's Nature Access metric (REFERENCE §6); not fed to the 2SFCA supply model or UMH (see DESIGN_NOTES §6.6). |
...
| Damage loss table | Blank — no per-building damage values. SA dashboard renders "Flood Volume Reduction" instead of monetized damage, matching Vibrant Land's reporting (Brief 33, Path C) | "(leaving blank)" per NatCap README; Vibrant Land (Guerry et al. 2023) reports flood mitigation as % volume reduction | ✅ Methodology aligned (Brief 33) — prototype matches NatCap's Vibrant Land per-city framing for SA |
...
| Child population raster | Census 2020 PL 94-171 under-18 (`P1_001N − P3_001N`), rasterized by the same uniform-block-spread as the total-pop raster (same blocks, same resolution, same CRS). `config.py:child_pop_file = data/sa/population/sa_child_pop_2020.tif`. Extent share 24.5 % (anchored in verify_baselines). | not in NatCap SA project bundle | ✅ Configured. Powers the Children's Nature Access metric (REFERENCE §6); not fed to the 2SFCA supply model or UMH (see DESIGN_NOTES §6.6). |
...
| Child population raster | ⏸️ `config.py:child_pop_file` is unset for Mpls Full. The Children's Nature Access metric (REFERENCE §6) reads "—" until a child_pop raster is generated via `scripts/data/download_census_pop.py` adapted to the Mpls-Full extent. |
