...
**Diagnostic — the 16.14 % nodata is a 30 m rasterization artifact, NOT a coverage hole (measured 2026-06-04; interpretation corrected after the follow-up sliver test — the original framing in commit `a3a8390` was wrong).** The measured magnitudes below are correct; the *interpretation* and *fix* in the first pass were not. The source is **already complete** — a full-county BCAD parcel pull (710,772 parcels, 2026-05-31, EOD-confirmed; `scripts/data/download_bexar_parcels.py`). The brief's hypothesized "fuller parcel source" **is the current source**, so a re-pull recovers ~nothing.
- **Where the nodata pop sits (measured, correct).** 98.6 % of the ~307k nodata population is on developed LULC (94 % on dense developed 22/23/24), ~0 % on open water. By area, one contiguous blob is 78 % of nodata *area* but only ~31 % of nodata *pop* (~96k); the other ~69 % (~211k) is fragmented across thousands of neighborhood-scale patches.
- **The decisive test — it's alignment noise, not absent parcels.** A nearest-classified distance transform shows nodata population is **99.1 % within 1 px (99.8 % within 2 px) of a classified parcel** — interstitial gaps where abutting BCAD parcels don't tile the 30 m grid, not pixels where a parcel is missing. Every top nodata component (including the 523k-px blob) has a **100 %-classified perimeter**. Only **~0.2 % (732 people) are genuine deep gaps** (>2 px from any classified pixel; likely water / federal interiors). So the earlier "developed-land coverage hole" reading was an artifact of 30 m rasterization, not a source gap.
- **Corrected fix — re-rasterization, NOT new data.** Re-rasterize the ownership layer with nearest-classified-fill (or sub-pixel rasterize → majority-downsample to 30 m). Recovers **~99 % of the nodata population**; **~95 % resolves to `private`** (matching the residential reality), the rest spread across unknown/city/etc. No new source, no license/fetch risk — a deterministic alignment pass in the existing build script. (Supersedes the original "re-pull a fuller source" recommendation.)
- **Blast radius.** Re-rasterizing the ownership raster re-snapshots **ownership-dependent baselines only** — the citywide population-weighted metrics read `pop_count_raster` (untouched), so they don't move. Smaller than the dasymetric `pop_count_raster` change rejected above.
...
**Why two surfaces.** Card-level validation captures methodology-vs-measurement nuance per metric (temperature can cite measured per-pixel parity; carbon is methodology adoption without per-pixel parity). Scenario-level provenance captures where the *scenario as a whole* came from (slider-driven exploration vs NatCap-published reference vs optimizer suggestion). Conflating the two would lose precision in either direction.
...
- Metric-aware tooltip on `≈ NatCap method`: temperature CAN cite measured per-pixel HMI parity (Brief 28b); carbon MUST NOT — four-pool methodology adoption (Brief 30) is alignment, not a per-pixel parity measurement. Overclaiming would misframe the reproduction posture.
