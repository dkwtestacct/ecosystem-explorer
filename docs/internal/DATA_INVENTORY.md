...
| SA — ownership 2-band raster (finer classes) | `data/sa/sa_ownership_2band_30m.tif` | Committed | EPSG:5070; **two-band int8** raster on the SA grid (1713 × 1984 @ 30 m). **Band 1 — ownership class enum:** `0=private`, `1=city`, `2=county`, `3=state-federal`, `4=school-university`, `5=unknown`; nodata `-1` (outside AOI). Public classes are written last per a priority order (low → high: unknown < private < state-federal < county < school-university < city) so public classes win at overlapping pixel edges. **Band 2 — vacant flag:** `0=not-vacant`, `1=vacant`; nodata `-1`. Same exempt-keyed vacancy rule as the legacy raster. Source: same 2026-05-31 BCAD pull, re-classified under the six-way rule from `docs/internal/OWNERSHIP_FEASIBILITY_PROFILING.md` (HOA → Private filter applied to the County branch; standalone `\bUSA\b` and `\bFEDERAL\b` dropped from the State-federal regex after spot-check surfaced ~1.3k ac of business-name false positives — "Boralis USA Inc", "Forestar (USA) Real Estate", "Home Depot USA Inc", "Security Service Federal Credit Union"). Rebuild command: `python scripts/data/download_bexar_parcels.py --reclassify-from <archived.gpkg>` (re-rasterizes from the archived 2026-05-31 GPKG without re-fetching BCAD). **Per-class polygon-Acres (full Bexar County, locked by `verify_baselines.py` reconciliation at ±0.5%):** private 606,379 ac · city 126,634 · state-federal 54,883 · school-university 6,430 · county 3,018 · unknown 1,735 · total 799,079 ac. **Per-class rasterized-AOI acres (locked at ±5%):** private ~507,110 · city ~41,044 · state-federal ~28,237 · school-university ~6,030 · county ~2,886 · unknown ~15,885. **Caveat — BCAD `Acres` field ≠ polygon geometry for two classes.** Reported `Acres` and geometry-derived acres reconcile within ~2% for private / county / school-university / state-federal, but diverge meaningfully for City (the master "CITY OF SAN ANTONIO" record reports ~115k aggregated acres against a much smaller polygon footprint — BCAD overcounts) and Unknown (parcels with Owner blank are mostly road rights-of-way with `Acres=0` but real geometry — ~16k ac of in-AOI ROW). The raster reflects geometry; the polygon-Acres reconciliation reflects the BCAD field. **Caveat — rule-derived, not ground-truthed.** Classes come from regex-parsing BCAD `Owner` + `Exempts`. No cross-check against an authoritative title registry. The filter is a planning screen. **Caveat — `school_university` is mixed public + private.** The bucket includes public institutions (ISDs, Alamo Community College District, UT System, TX A&M System) AND private institutions (Trinity University, St. Mary's University, Our Lady of the Lake University) — they're all caught by the same `\b(UNIVERSITY\|COLLEGE\|INDEPENDENT SCHOOL\|...)\b` rule. Treat `school_university` as "school district + university campuses" — NOT as a government category. For the "Publicly-owned land" rollup (`OWNERSHIP_MODES['public']`), `school_university` is intentionally excluded — only `city + county + state_federal` qualify, so a private Trinity campus doesn't get counted as publicly-available planning land. School-university remains a first-class selectable filter on its own; it just isn't a default member of `public`. |
