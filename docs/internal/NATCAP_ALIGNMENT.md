...
- **Per-pixel parity against canonical InVEST** — the prototype reimplements UCM / UFR / UNA / UMH / Carbon in numpy and validates each against `natcap.invest.*.execute()` on matched inputs (`compare_*_invest.py` harnesses). HMI MAE = 0.0000 / r = 1.0000 (UCM, Brief 28b); UMH MAE ≈ 0 / r = 1.000000 on aligned input (Brief B); **SA Carbon four-pool: per-pixel MAE ≈ 0 (3.3e-7 Mg C) / r = 1.0 vs `natcap.invest` 3.19.0 in matched units** (`compare_carbon_sa_fourpool_invest.py`, Relay 69; result in `comparisons/carbon_sa_fourpool_parity.csv` — see §6). This is the prototype's anchored reproduction claim.
...
|---|---|---|
| **`validated`** | Per-pixel parity measured against canonical `natcap.invest.*.execute()` | UCM, UNA, UMH, Carbon |
| **`methodology_aligned`** | Canonical method, no per-pixel parity check | UFR |

Each entry includes the reference InVEST version (3.19.0) and a per-model notes string sourced from this file. The two-state taxonomy answers a yes/no question per model (does the prototype's output match canonical?), while the §2 four-state badge captures per-metric methodology nuance (e.g. temperature and carbon both cite measured per-pixel parity, while UFR stays methodology-aligned). They are distinct surfaces for distinct audiences.
