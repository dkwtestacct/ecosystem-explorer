...
- **Per-pixel parity against canonical InVEST** — the prototype reimplements UCM / UFR / UNA / UMH / Carbon in numpy and validates each against `natcap.invest.*.execute()` on matched inputs (`compare_*_invest.py` harnesses). HMI MAE = 0.0000 / r = 1.0000 (UCM, Brief 28b); UMH per-pixel parity — **MN** MAE ≤ 1.1e-9 / r = 1.000000, **SA** MAE ≤ 2.3e-6 / r ≥ 0.99875 / |Δtotal| ≤ 0.15% (Brief B; see §6); **SA Carbon four-pool: per-pixel MAE ≈ 0 (3.3e-7 Mg C) / r = 1.0 vs `natcap.invest` 3.19.0 in matched units** (`compare_carbon_sa_fourpool_invest.py`, Relay 69); **UFR runoff-retention index: per-pixel MAE ≈ 0 (5e-8) / r = 1.0 vs `natcap.invest` UFRM 3.19.0** (`compare_ufr_invest.py`, Relay 71). Results in `comparisons/*_parity.csv`; see §6. This is the prototype's anchored reproduction claim. (The UFR *retention index* is the per-pixel-validated reading; the lumped Flood Index `100 − mean_CN` and Runoff Volume stay aligned-method.)
