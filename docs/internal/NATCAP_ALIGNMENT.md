...
- **Per-pixel parity against canonical InVEST** — the prototype reimplements UCM / UFR / UNA / UMH / Carbon in numpy and validates each against `natcap.invest.*.execute()` on matched inputs (`compare_*_invest.py` harnesses). HMI MAE = 0.0000 / r = 1.0000 (UCM, Brief 28b); UMH MAE ≈ 0 / r = 1.000000 on aligned input (Brief B). **Carbon is the exception: methodology-aligned only — no grounded per-pixel parity has been measured.** The one committed carbon harness (`validation/compare_carbon_invest.py`) is an MN single-pool-proxy vs InVEST four-pool-stock AOI-sum check, not a per-pixel parity test (see §6). The measured-parity claim is anchored on UCM / UNA / UMH.
...
1. **Per-metric evidence varies within "≈ NatCap method."** Temperature can cite measured per-pixel parity (HMI MAE ≈ 0, Brief 28b); carbon is four-pool methodology adoption (Brief 30) with **no measured per-pixel parity** — do not imply parity for carbon. (The only committed carbon comparison is an AOI-sum check on an MN single-pool proxy, explicitly not a parity test — see §6.) The per-card tooltip surfaces this nuance.
...
**Carbon — methodology-aligned, no grounded per-pixel parity (corrected Relay 68).** Carbon stays `methodology_aligned`: there is **no committed SA four-pool per-pixel parity result.** An earlier record (commit 463912b, Relays 61/66) asserted a per-pixel spatial r = 1.00000000 / MAE 121 Mg C against `natcap.invest` 3.16.2, but that number came from an **uncommitted scratch script (since deleted)** and is **not reproducible from anything in the repo** — it has been withdrawn and must not stand as a measured result. The one committed carbon comparison, `validation/compare_carbon_invest.py`, measures something different: an **MN single-pool annual-sequestration proxy vs InVEST four-pool stock,** compared as an **AOI-sum scalar** — its own header states the two measure different things, so it is *not* a parity test. It shows close AOI-sum agreement (app 2596.0 vs InVEST 2595.85 t CO₂e, 0.01 %, `natcap.invest` 3.16.2). **Elevation to `validated`** would require authoring a committed SA four-pool **per-pixel** parity harness, run against `natcap.invest` 3.19.0 in matched units (per-pixel MAE + r) with a non-vacuous perturbation guard — **deferred.**
