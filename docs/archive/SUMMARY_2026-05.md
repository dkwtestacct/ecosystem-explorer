...
| `BASELINE_HM` (= mean HMI) | 0.1859 | 0.1600 | 0.2866 | Mean of the canonical HMI raster |
...
- **InVEST UCM divergences from canonical** — the HMI raster is canonical (validated at MAE = 0.0000 vs `natcap.invest.urban_cooling_model.execute()`); the one remaining divergence is that energy savings are aggregated per pixel rather than per building (skipping the `t_air_average_radius` aggregation step). This affects *Cooling Energy Savings* dollar magnitudes but not *Temperature Change*. See `data/invest/cooling/UCM_AUDIT.md` for the full divergence log.
...
`SCENARIO_SCHEMA_VERSION = 27` invalidates cached lookup tables across changes to the metric schema. Per-city caches don't collide because the cache key includes the data directory and filename arguments to `load_data`.
