...
| Damage loss table | Blank — no per-building damage values. SA dashboard renders "Flood Retention" (% volume reduction) instead of monetized damage, matching Vibrant Land's reporting (Brief 33, Path C) | "(leaving blank)" per NatCap README; Vibrant Land (Guerry et al. 2023) reports flood mitigation as % volume reduction | ✅ Methodology aligned (Brief 33) — prototype matches NatCap's Vibrant Land per-city framing for SA |
