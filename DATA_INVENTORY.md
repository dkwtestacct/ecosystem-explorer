...
├── precomputed/                 # Orphaned: dead .npy distance-transform cache (see §11)
│   └── minneapolis_full_mn/     (7 dead .npy files, untracked)
...
`du -sh data/*/`: cooling 296 K · flood 320 K · invest 3.2 M · minneapolis_expanded 115 M · osm **7.2 G** · population 971 M · precomputed 6.1 M · sa **1.3 G**. `du -sh data/sa/*/`: cooling 835 M · flood 27 M · natcap_2024 321 M · population 396 K · tracts_bexar 51 M.
...
| Path | Status |
|---|---|
| `data/precomputed/minneapolis_full_mn/nature_distance_*.npy` | **Orphaned dead data.** 7 untracked `.npy` files generated by the diagnostic before Brief 9 deleted the `_compute_access_score_raster_pure` consumer. Equivalent already-tracked files under `data/precomputed/minneapolis_mn/` and `data/sa/precomputed/` were deleted in Brief 9's first commit; this `minneapolis_full_mn/` instance was left untracked and remains on disk. Scheduled for cleanup in a future commit. |
