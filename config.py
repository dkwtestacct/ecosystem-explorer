...
CITIES = {
    ...
    'San Antonio, TX': {
        ...
        # which takes ~32 s of the ~60 s SA cold start (after Lever 1's
        # Fast-mode Fast→dense CSV switch landed in a028b51). With these
        # three files present, Phase 8 reads them in <1 s. Generated by
        # `python3 precompute_buildings.py --city 'San Antonio, TX'`. The
        # gate runs a fresh rasterize and asserts byte-identity vs disk —
        # if the source `buildings_file` changes, the assertion fails until
        # the precompute is re-run. Cities without these keys use the live
        # rasterize path unchanged (MN, Mpls Full). Committed as plain git
        # (no LFS) at ~10 MB total — within the 50 MB GitHub soft limit.
        'buildings_precomputed_file':       'data/sa/sa_buildings_precomputed.tif',
        'buildings_type_precomputed_file':  'data/sa/sa_buildings_type_precomputed.tif',
        'buildings_precomputed_meta_file':  'data/sa/sa_buildings_precomputed_meta.json',
