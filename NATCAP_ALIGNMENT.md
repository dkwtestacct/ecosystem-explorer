...
| Preventable MH Cases | `(1 − RR) × BIR × pop`; NE via Gaussian-smoothed synthetic NDVI. **Validated against `natcap.invest.urban_mental_health.execute()` v3.19.0** (`compare_umh_invest.py`): city-wide totals match to **~1.3–1.5 %**, but per-pixel **Pearson r ≈ 0.95 (MN) – 0.98 (SA)** because canonical uses a uniform **buffer-mean** NE kernel whereas the app uses a **Gaussian** (σ = radius/px). Validation is on the synthetic NDVI proxy, not satellite NDVI. | Urban Mental Health v3.19.0 (same PC formula; different NE kernel) | Implemented | Medium |
| Avoided MH Costs | Preventable cases × per-case cost-of-illness (inherits the Preventable MH Cases validation above — linear in cases) | Urban Mental Health (cost module) | Implemented | Medium |
